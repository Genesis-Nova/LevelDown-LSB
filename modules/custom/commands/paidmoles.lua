...
commandObj.onTrigger = function(player)
	...
	local now = os.time()
	local cooldown = 60 * 60 -- 60 minutes in seconds
	local currentZoneID = player:getZoneID()
	
	--Check if in restricted zone
	if restrictedZones[currentZoneID] then
    player:printToPlayer("You cannot use !paidmoles in this zone. Please go to an area where you can engage monster normally.")
    return
	end
    ...
    if not player:delGil(cost) then
        player:printToPlayer("An error occurred while trying to deduct gil.")
        return
    end

    player:printToPlayer(string.format("You have paid %i gil to summon the Moles!", cost))
	
	-- Set 60-minute cooldown
	zone:setLocalVar("PaidMolesLastUsed", os.time())

		--Text to output to players in area
		player:printToArea(string.format('%s has paid the moles to appear, but they have stolen our eggs!', name), xi.msg.channel.SYSTEM_3, 0)
		player:printToArea('Help us stop them, we have 10 minutes before they disappear back into the ground!', xi.msg.channel.SYSTEM_3, 0)
		player:printToArea(string.format('Remember to tip %s!', name), xi.msg.channel.SYSTEM_3, 0)
		player:printToArea('Be aware: there is a 60-minute cooldown to restart the event in this zone.', xi.msg.channel.SYSTEM_3, 0)
    ...
    for i = 1, 10 do 
        ...
        local mob = zoneOrInstanceObj:insertDynamicEntity({
            ...
            onMobDeath = function(mob, playerArg, optParams) end,
            onMobDespawn = function(mob, playerArg, optParams) end,
